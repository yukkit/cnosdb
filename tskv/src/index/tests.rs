...
async fn test_index_add_del() {
    ...
    let mut sids_2 = index.get_series_id_list(&table, &[]).await.unwrap();
    sids_2.sort();
    ...
    let sids_3 = index.get_series_id_list(&table, &[]).await.unwrap();
...
async fn test_index_id_list() {
    ...
    let list = index.get_series_id_list("table_test", &tags).await.unwrap();
