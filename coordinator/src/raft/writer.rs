...
use protos::kv_service::{DeleteFromReplicaRequest, WriteReplicaRequest};
...
impl RaftWriter {
    ...
    pub async fn delete_from_table_on_replica(
        &self,
        ...
        if leader_id == node_id && self.kv_inst.is_some() {
            let span_recorder = span_recorder.child("delete on local node or forward");
            let result = self
                .delete_on_local_or_forward(
                    request,
                    tenant,
                    db_name,
                    replica,
            ...
            debug!("delete on local {} {:?} {:?}", node_id, replica, result);

            return result;
        }

        let span_recorder = span_recorder.child("delete on remote node");
        let result = self
            .delete_on_remote(leader_id, request, span_recorder.span_ctx())
            .await;
        debug!("delete on remote {} {:?} {:?}", leader_id, replica, result);
        ...
        if let Err(CoordinatorError::FailoverNode { .. }) = result {
            for vnode in replica.vnodes.iter() {
                ...
                let result = self
                    .delete_on_remote(vnode.node_id, request, span_recorder.span_ctx())
                    .await;
                debug!(
                    "try write to remote {} {:?} {:?}",
    ...
    pub async fn delete_on_local_or_forward(
        &self,
        request: DeleteFromReplicaRequest,
        tenant: &str,
        db_name: &str,
        replica: &ReplicationSet,
        ...
        {
            let node_id = self
                .process_leader_change(tenant, leader_vnode_id, request.replica_id)
                .await?;
            self.delete_on_remote(node_id, request, span_ctx).await
        } else {
            result
        }
    }

    async fn delete_on_remote(
        &self,
        leader_id: u64,
        request: DeleteFromReplicaRequest,
        span_ctx: Option<&SpanContext>,
        ...
        if use_time > 200 {
            debug!(
                "delete on node:{}, use time too long {}",
                leader_id, use_time
            )
        }
    ...
    pub async fn write_to_local_or_forward(
        ...
        }) = result
        {
            let node_id = self
                .process_leader_change(tenant, leader_vnode_id, request.replica_id)
                .await?;
            self.write_to_remote(node_id, request, span_ctx).await
        } else {
            result
        }
    }
