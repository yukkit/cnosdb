...
use super::header::{
    Header, APPLICATION_CSV, APPLICATION_JSON, APPLICATION_NDJSON, APPLICATION_PREFIX,
    APPLICATION_STAR, APPLICATION_TSV, CONTENT_TYPE, STAR_STAR,
};
...
pub async fn wrap_result(header: Header, res: &mut QueryHandle) -> Result<Response, HttpError> {
    let fmt = ResultFormat::try_from(header.get_accept())?;

    let batches = fetch_record_batches(res)
        .await
        .map_err(|e| HttpError::FetchResult {
            reason: format!("{}", e),
        })?;
